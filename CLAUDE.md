...
2. **Реестр спецдвижков `SPECIAL_ENGINE_RUNNERS` объявлен В ДВУХ МЕСТАХ** — `main.py` (CLI)
   и `src/api/server.py` (веб). Добавляя движок, править **оба**, иначе тип будет работать
   из командной строки и молчать в интерфейсе. Так уже терялись 6 типов (закрыто 24.08.2026).
