...
# Один блок RULE. Поля `target_line` и `exclusions_line` могут быть пустыми —
# пустая строка не добавляет лишнюю пустую строку в финальный текст, т.к. они
# уже собираются через \n-join из непустых элементов.
RULE_BLOCK_TEMPLATE = """\
### RULE {index} — {title}
{target_line}
    ...
    *,
    ...
    Логика:
        ...
        4. Делает прямой chat.completions.create с Qwen-специфичными параметрами
           (top_p, presence_penalty, top_k, min_p, repetition_penalty, chat_template_kwargs.enable_thinking=False).
