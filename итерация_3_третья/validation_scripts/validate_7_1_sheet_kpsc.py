#!/usr/bin/env python3
"""
Валидатор 7.1: Наличие и заполнение листа "КПСЦ"
Источник ТЗ: ТЗ_Валидация_КПСЦ_v2_упрощенная.md, Раздел 7, Проверка 7.1
"""
import os
import json
import argparse
...
def load_data(parser_outputs_dir: Path) -> dict:
    ...
    # Проверяем наличие файлов от листа КПСЦ
    header_file = parser_outputs_dir / "kpsc_header_v2.json"
    table1_file = parser_outputs_dir / "kpsc_table1_v2.json"

    files_exist = {
        "kpsc_header_v2.json": header_file.exists(),
        "kpsc_table1_v2.json": table1_file.exists()
    }
...
def extract_relevant_data(data: dict) -> dict:
    ...
    return {
        "files_status": str(files_exist),
        "has_data": has_data,
        "files_count": sum(files_exist.values())
    }
