#!/usr/bin/env python3
"""
Валидатор 7.7: Наличие и заполнение листа "Перечень проблем по Диаграмме Спагетти"
Источник ТЗ: ТЗ_Валидация_КПСЦ_v2_упрощенная.md, Раздел 7, Проверка 7.7
"""
import os
import json
import argparse
...
def extract_relevant_data(data: dict) -> dict:
    ...
    return {
        "file_exists": True,
        "has_rows": len(rows) >= 2,  # минимум заголовок + 1 строка данных
        "rows_count": len(rows)
    }
