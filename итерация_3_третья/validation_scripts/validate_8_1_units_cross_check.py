#!/usr/bin/env python3
"""
Валидатор 8.1: Соответствие единиц измерения между листами "Показатели" и "КПСЦ"
Источник: Колонка J, лист Показатели, ячейка J2
"""
import os
import json
import argparse
...
def extract_relevant_data(data: dict) -> dict:
    ...
    # Извлекаем показатели из листа "Показатели"
    ...
    for row in pokazateli_rows:
        ...
        unit = cells_dict.get(4, "")  # Колонка D - единица измерения
        if unit:
            pokazateli_units[str(pokazatel_name).strip().lower()] = str(unit).strip()

    # Извлекаем показатели из блока "Расчет ВПП" на листе КПСЦ
    ...
    for i, row in enumerate(kpsc_rows):
        ...
        # Колонка D или AA (col=27) - единица измерения
    ...
    for pokazatel, unit_pokazateli in pokazateli_units.items():
        if pokazatel in kpsc_units:
            ...
            if unit_pokazateli.lower() != unit_kpsc.lower():
                mismatches.append({
                    "pokazatel": pokazatel,
                    "unit_pokazateli": unit_pokazateli,
                    "unit_kpsc": unit_kpsc
                })

    return {
        "files_exist": True,
        "pokazateli_count": len(pokazateli_units),
        "kpsc_count": len(kpsc_units),
        "pokazateli_units": pokazateli_units,
        "kpsc_units": kpsc_units,
...
# ToDo : узнать у эксперта что именно сравнивается (и имена строк с их измерениями в "Расчет ВПП" в листах "Показатели" и "КПСЦ")
