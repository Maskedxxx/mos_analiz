#!/usr/bin/env python3
"""
Валидатор 7.8: Наличие и заполнение листа "Расчет такта" / "Расчет времени такта"
Источник ТЗ: ТЗ_Валидация_КПСЦ_v2_упрощенная.md, Раздел 7, Проверка 7.8
"""
import os
import json
import argparse
...
def load_data(parser_outputs_dir: Path) -> dict:
    ...
    # Проверяем наличие отдельного файла для расчета такта
    # Если его нет, проверяем поле takt_time в заголовке КПСЦ
