#!/usr/bin/env python3
"""
Парсер таблицы "6. Определение времени выполнения сменного задания ОПЕРАТОРАМИ, час".
Ширина до Z включительно (захватываем аннотации в Y:Z), окончание перед секцией "7.".
"""
import argparse
import json
...
SECTION_PHRASE = "определение времени выполнения сменного задания оператор"
NEXT_PREFIX = "7."
# до Z (26), чтобы сохранить аннотации в слияниях Y:Z
FORCED_RIGHT_COL = 26
...
def find_section_anchor(ws, phrase: str) -> Optional[Tuple[int, int]]:
    ...
    for row in ws.iter_rows():
        for cell in row:
            v = cell.value
            if isinstance(v, str) and p in v.lower() and v.strip().startswith("6"):
...
def build_payload(xlsx_path: Path, sheet: str):
    ...
    if not anchor:
        raise SystemExit("Не найден заголовок '6. Определение времени выполнения сменного задания ОПЕРАТОРАМИ, час'")
...
def main():
    ap = argparse.ArgumentParser(description="Парсер таблицы 6 (сменное задание операторов)")
