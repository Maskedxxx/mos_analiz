#!/usr/bin/env python3
"""
Парсер таблицы "5. Определение потерь на брак" (лист КПСЦ).
Границы по ширине до колонки X включительно; конец перед секцией "6.".
"""
import argparse
import json
...
SECTION_PHRASE = "определение потерь на брак"
NEXT_PREFIX = "6."
FORCED_RIGHT_COL = 24
...
def find_section_anchor(ws, phrase: str) -> Optional[Tuple[int, int]]:
    ...
    for row in ws.iter_rows():
        for cell in row:
            v = cell.value
            if isinstance(v, str) and p in v.lower() and v.strip().startswith("5"):
...
def build_payload(xlsx_path: Path, sheet: str):
    ...
    if not anchor:
        raise SystemExit("Не найден заголовок '5. Определение потерь на брак'")
...
def main():
    ap = argparse.ArgumentParser(description="Парсер таблицы 5 (потери на брак)")
