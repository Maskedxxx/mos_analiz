#!/usr/bin/env python3
"""
Парсер таблицы "7. Определение доступного рабочего времени" на листе КПСЦ.
Ширина до X включительно; конец перед секцией "8." (если есть) либо по пустой строке.
"""
import argparse
import json
...
SECTION_PHRASE = "определение доступного рабочего времени"
NEXT_PREFIX = "8."
FORCED_RIGHT_COL = 24
...
def find_section_anchor(ws, phrase: str) -> Optional[Tuple[int, int]]:
    ...
    for row in ws.iter_rows():
        for cell in row:
            v = cell.value
            if isinstance(v, str) and p in v.lower() and v.strip().startswith("7"):
...
def build_payload(xlsx_path: Path, sheet: str):
    ...
    if not anchor:
        raise SystemExit("Не найден заголовок '7. Определение доступного рабочего времени'")
...
def main():
    ap = argparse.ArgumentParser(description="Парсер таблицы 7 (доступное рабочее время)")
