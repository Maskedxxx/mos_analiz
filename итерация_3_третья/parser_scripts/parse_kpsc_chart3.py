#!/usr/bin/env python3
"""
Парсер третьей диаграммы на листе "КПСЦ" (после таблицы 9).
Использует chart3.xml (rId12 в drawing2), разворачивает серии и точки.
"""
import argparse
import json
import posixpath
...
def parse_chart(xlsx: Path, sheet_name: str):
    ...
    bbox, chart_target = find_chart_anchor_and_target(xlsx, drawing_path, target_rid="rId12")
...
def main():
    ap = argparse.ArgumentParser(description="Парсер chart3 после таблицы 9")
