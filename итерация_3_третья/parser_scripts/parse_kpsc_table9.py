#!/usr/bin/env python3
"""
Парсер таблицы "9. Загрузка участков производства, секунд на 1 партию (6 бочек - 1200 кг)".
Границы по ширине до W включительно (колонка 23), конец — перед секцией "10." или по первой пустой строке после данных.
"""
import argparse
import json
...
SECTION_PHRASE = "загрузка участков производства"
NEXT_PREFIX = "10."
FORCED_RIGHT_COL = 23  # W
...
def find_section_anchor(ws, phrase: str) -> Optional[Tuple[int, int]]:
    ...
    for row in ws.iter_rows():
        for cell in row:
            v = cell.value
            if isinstance(v, str) and p in v.lower() and v.strip().startswith("9"):
...
def build_payload(xlsx_path: Path, sheet: str):
    ...
    if not anchor:
        raise SystemExit("Не найден заголовок '9. Загрузка участков производства...'")
...
def main():
    ap = argparse.ArgumentParser(description="Парсер таблицы 9 (загрузка участков)")
