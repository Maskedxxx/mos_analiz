#!/usr/bin/env python3
"""
Парсер таблицы "8. Определение времени выполнения сменного задания ОБОРУДОВАНИЕМ, час" на листе КПСЦ.
Особенности:
  - ширина до X включительно;
  - справа есть слияния Y:Z с аннотациями — сохраняем merge_range и значение (не обрезаем);
  - окончание перед секцией "9." или по первой пустой строке после данных.
"""
import argparse
import json
...
SECTION_PHRASE = "определение времени выполнения сменного задания оборуд"
NEXT_PREFIX = "9."
FORCED_RIGHT_COL = 26  # Z (чтобы захватить слияния Y:Z)
...
def find_section_anchor(ws, phrase: str) -> Optional[Tuple[int, int]]:
    ...
    for row in ws.iter_rows():
        for cell in row:
            v = cell.value
            if isinstance(v, str) and p in v.lower() and v.strip().startswith("8"):
...
def build_payload(xlsx_path: Path, sheet: str):
    ...
    if not anchor:
        raise SystemExit("Не найден заголовок '8. Определение времени выполнения сменного задания ОБОРУДОВАНИЕМ, час'")
...
def main():
    ap = argparse.ArgumentParser(description="Парсер таблицы 8 (сменное задание оборудованием)")
