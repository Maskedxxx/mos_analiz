#!/usr/bin/env python3
"""
Парсер таблицы "4. Определение времени простоев" на листе КПСЦ.
Границы: до колонки X включительно; конец перед секцией "5.".
"""
import argparse
import json
...
SECTION_PHRASE = "определение времени простоев"
NEXT_PREFIX = "5."
FORCED_RIGHT_COL = 24
...
def find_section_anchor(ws, phrase: str) -> Optional[Tuple[int, int]]:
    ...
    for row in ws.iter_rows():
        for cell in row:
            v = cell.value
            if isinstance(v, str) and p in v.lower() and v.strip().startswith("4"):
...
def build_payload(xlsx_path: Path, sheet: str):
    ...
    if not anchor:
        raise SystemExit("Не найден заголовок '4. Определение времени простоев'")
...
def main():
    ap = argparse.ArgumentParser(description="Парсер таблицы 4 (простой)")
