#!/usr/bin/env python3
"""
Парсер второй диаграммы на листе "КПСЦ" (между таблицами 8 и 9).
Читает chart2.xml из drawing2, раскручивает серии и точки.
"""
import argparse
import json
import posixpath
...
def find_chart_anchor_and_target(xlsx: Path, drawing_path: str, target_rid: str) -> Tuple[dict, str]:
    with zipfile.ZipFile(xlsx) as z:
    ...
    for anc in root.findall("./", NS):
        gf = anc.find("xdr:graphicFrame", NS)
        if gf is None:
            continue
        chart_el = gf.find(".//c:chart", {"c": NS["c"], "r": NS["r"]})
        if chart_el is None:
            continue
        ...
        return bbox, target
    raise SystemExit("chart with given rId not found")
...
def read_values(ws, refs: List[Tuple[int, int]]) -> List[dict]:
    # ищем строку шапки (где в кол. B «Показатель») выше минимальной строки
...
def parse_chart(xlsx: Path, sheet_name: str):
    _, drawing_path = read_sheet_rel_paths(xlsx, sheet_name)
    bbox, chart_target = find_chart_anchor_and_target(xlsx, drawing_path, target_rid="rId2")
...
def main():
    ap = argparse.ArgumentParser(description="Парсер chart2 между таблицами 8 и 9")
