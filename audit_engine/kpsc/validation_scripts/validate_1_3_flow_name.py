#!/usr/bin/env python3
"""
Валидатор 1.3: Наличие названия потока в формате "имя потока"
Источник ТЗ: ТЗ_Валидация_КПСЦ_v2_упрощенная.md, Раздел 1, Проверка 1.3
"""
import os
import json
import argparse
...
def extract_relevant_data(data: dict) -> dict:
    ...
    return {
        "flow_name": data["fields"].get("flow_name", "")
    }
...
def main():
    ...
    parser.add_argument(
        "--log-file",
        type=Path,
        help="Путь к файлу лога (если не указан, используется validation_logs/validate_X_Y.log)"
    )
    ...
    if args.verbose:
        log_step(log_file, 1, "Загрузка правила проверки",
    ...
    if args.verbose:
        log_step(log_file, 2, "Загрузка данных",
                 f"Source file: {args.parser_outputs / 'kpsc_header_v2.json'}\n"
    ...
    if args.verbose:
        log_step(log_file, 6, "Финальный результат",
                 f"Status: {result['status']}\n"
                 f"Discrepancy: {result.get('discrepancy', 'нет')}\n"
