...
# -*- coding: utf-8 -*-
"""
Оркестратор валидации КПСЦ.

Запускает 9 парсеров и 25 валидаторов, собирает результаты.
Адаптация packaged_services/kpsc_validation/run_all_validations.py:
вместо subprocess используется прямой импорт модулей.
"""
...
# === Реестр парсеров: модуль → функция parse() ===
PARSER_MODULES = [
    "audit_engine.kpsc.parser_scripts.parse_kpsc_header",
    "audit_engine.kpsc.parser_scripts.parse_kpsc_table1",
    "audit_engine.kpsc.parser_scripts.parse_legend",
    "audit_engine.kpsc.parser_scripts.parse_loss_digitization",
    "audit_engine.kpsc.parser_scripts.parse_pa1_chart",
    "audit_engine.kpsc.parser_scripts.parse_pa1_table",
    "audit_engine.kpsc.parser_scripts.parse_pokazateli",
    "audit_engine.kpsc.parser_scripts.parse_spaghetti_sheet",
    "audit_engine.kpsc.parser_scripts.parse_spaghetti_problems",
]
...
def get_validator_module_name(rule_index: str) -> str:
    ...
    try:
        ...
        if output_file.exists():
            ...
            result_data = {
                "rule_index": rule_index,
                "status": "ERROR",
                "discrepancy": f"Выходной файл не создан. STDERR: {result.stderr[:300]}"
            }
