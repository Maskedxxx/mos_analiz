...
# -*- coding: utf-8 -*-
"""
Модуль валидации «Карточка проекта» (doc_type: kartochka_proekta).

Точка входа: run(args) — вызывается из run_audit.py для doc_type="kartochka_proekta".
Пайплайн: 3 парсера → 11 валидаторов (параллельно) → Excel-отчёт.
"""
...
def run(args) -> AuditResult:
    ...
    # Переменная среды для валидаторов
    os.environ["VALIDATION_RULES_PATH"] = str(rules_path)

    # === Шаг 1: Парсинг (3 парсера) ===
