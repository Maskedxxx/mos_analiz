...
@register_preprocessor("prikaz_comp_ppu", "заголовок_город")
def normalize_title(text: str) -> str:
    """
    Нормализует чанк заголовок_город для сравнения с шаблоном (правило #1).

    Vision извлекает элементы в разном порядке (target: ПРИКАЗ→заголовок→город,
    template: ПРИКАЗ→город→заголовок). Убираем вариативные части,
...
def normalize_text_for_rule3(text: str) -> str:
    """
    Нормализует текст приказа о конкурсах ППУ для сравнения с шаблоном (правило #3).
    ...
    #    Target: "возложить на Рассейкина В.А., заместителя руководителя по качеству."
    ...
    # Нормализация шаблонных плейсхолдеров: <ФИО> и <должность> → [ДОЛЖНОСТЬ_ФИО]
    # Без этого target и template содержат разные форматы подстановок
    text = re.sub(
