...
# START_SOURCE_VISION_PARSER_EXTRACTOR
# PURPOSE: Inlined source from audit_engine/vision_parser/extractor.py.
...
class VisionExtractor:
    ...
    def extract(self, pdf_path: str, chunks: List[ChunkConfig], chunk_filter: Optional[str]=None) -> Dict[str, str]:
        """
    ...
    async def _extract_all_chunks(self, images: List[Image.Image], chunks: List[ChunkConfig], total_pages: int) -> Dict[str, str]:
        """
    ...
    async def _extract_chunk_async(self, images: List[Image.Image], chunk: ChunkConfig, total_pages: int) -> str:
        """
        ...
        if len(valid_indices) == 1:
            ...
            return await self._call_vision_api_async(images[page_idx], chunk.prompt, f'{chunk.name}', page_idx)
        tasks = []
        for i, page_idx in enumerate(valid_indices):
            page_num = page_idx + 1
            page_prompt = f'[СТРАНИЦА {page_num}]\n\n{chunk.prompt}'
            tasks.append(self._call_vision_api_async(images[page_idx], page_prompt, f'{chunk.name}_p{page_num}', page_idx))
    ...
    def _pdf_to_images(self, pdf_path: str) -> List[Image.Image]:
        ...
        try:
            ...
            raise ExtractionError('pdf2image не установлен. Установите:\n  pip install pdf2image\n  # И poppler:\n  macOS: brew install poppler\n  Linux: apt install poppler-utils')
        try:
    ...
    async def _call_vision_api_async(self, image: Image.Image, prompt: str, chunk_name: str, page_idx: int) -> str:
        """
        ...
        content = [{'type': 'image_url', 'image_url': {'url': f'data:image/png;base64,{base64_image}', 'detail': self.config.detail}}, {'type': 'text', 'text': prompt}]
        if self.vision_prompts_dir:
            self._log_vision_prompt(chunk_name, page_idx, prompt)
        try:
            response = await self.client.chat.completions.create(model=self.config.model, messages=[{'role': 'system', 'content': self.SYSTEM_PROMPT}, {'role': 'user', 'content': content}], max_tokens=8096, temperature=0.0)
...
def extract_from_pdf(pdf_path: str, chunks_config_path: str, vision_config: Optional[VisionConfig]=None, chunk_filter: Optional[str]=None) -> Dict[str, str]:
    """
...
vision_parser_extractor_module = SimpleNamespace(ExtractionError=ExtractionError, VisionExtractor=VisionExtractor, extract_from_pdf=extract_from_pdf)

# END_SOURCE_VISION_PARSER_EXTRACTOR
...
# START_SOURCE_VISION_PARSER
# PURPOSE: Inlined source from audit_engine/vision_parser/__init__.py.
vision_parser___all__ = ['VisionParser', 'ChunkConfig', 'VisionConfig', 'load_chunks_config', 'load_vision_config', 'DocumentConverter', 'VisionExtractor', 'ChunkAggregator', 'ConversionError', 'ExtractionError']
...
class VisionParser:
    ...
    def __init__(self, chunks_config: str, vision_config: VisionConfig=None, temp_dir: str=None, log_dir: str=None):
        """
    ...
    def parse(self, file_path: str, chunk_filter: Optional[str]=None) -> Dict[str, Any]:
        ...
        try:
            ...
            raw_chunks = self.extractor.extract(pdf_path, self.chunks_config, chunk_filter=chunk_filter)
            result = self.aggregator.aggregate(raw_chunks, str(input_path))
            return result
        finally:
            self._cleanup()

    def parse_pages(self, file_path: str, pages: List[int], prompt: str) -> str:
        """
        ...
        Returns:
            Извлечённый текст
        """
        temp_chunk = ChunkConfig(name='_temp', pages=pages, prompt=prompt)
        input_path = Path(file_path)
        try:
            pdf_path = self.converter.convert(str(input_path))
            if pdf_path != str(input_path.absolute()):
                self._temp_pdf = pdf_path
            raw_chunks = self.extractor.extract(pdf_path, [temp_chunk])
...
def parse_document(file_path: str, chunks_config: str, vision_config: VisionConfig=None) -> Dict[str, Any]:
    """
...
vision_parser_module = SimpleNamespace(__all__=vision_parser___all__, __version__=__version__, VisionParser=VisionParser, parse_document=parse_document)

# END_SOURCE_VISION_PARSER
...
# START_SOURCE_OCR_PARSER_EXTRACTOR
# PURPOSE: Inlined source from audit_engine/ocr_parser/extractor.py.
ocr_parser_extractor_log = logging.getLogger(__name__)
...
class OcrExtractor:
    ...
    def __init__(self, base_url: str='http://localhost:8010/v1/', model: Optional[str]=None, prompt: str='', dpi: int=200, log_dir: Optional[str]=None):
        """
    ...
    def _detect_model(self) -> str:
        ...
        try:
            ...
            if models.data:
                ...
                ocr_parser_extractor_log.info(f'OCR модель (авто): {model_id}')
                return model_id
    ...
    def detect_blank_pages(self, pdf_path: str) -> Tuple[int, Set[int]]:
        ...
        for i, img in enumerate(images):
            if self._is_blank_page(img):
                blanks.add(i)
                ocr_parser_extractor_log.info(f'  стр.{i + 1}: пустая (пропускаем)')
        if blanks:
            ocr_parser_extractor_log.info(f'Пустых страниц: {len(blanks)} из {total} (LibreOffice артефакты)')
        return (total, blanks)
    ...
    def extract_pages(self, pdf_path: str, page_indices: List[int]) -> Dict[int, str]:
        ...
        if not unique_indices:
            ocr_parser_extractor_log.warning(f'Нет валидных страниц из {page_indices} (всего {total_pages})')
            return {}
        ocr_parser_extractor_log.info(f'OCR: {len(unique_indices)} страниц из {total_pages}, модель={self.model}')
        result = asyncio.run(self._ocr_pages_async(images, unique_indices))
        return result

    async def _ocr_pages_async(self, images: List[Image.Image], page_indices: List[int]) -> Dict[int, str]:
        """
        ...
        for idx, result in zip(page_indices, results):
            page_num = idx + 1
            if isinstance(result, Exception):
                ocr_parser_extractor_log.error(f'OCR страница {page_num}: {result}')
    ...
    async def _ocr_single_page(self, image: Image.Image, page_idx: int) -> str:
        ...
        page_num = page_idx + 1
        base64_image = self._image_to_base64(image)
        content = [{'type': 'image_url', 'image_url': {'url': f'data:image/png;base64,{base64_image}', 'detail': 'high'}}, {'type': 'text', 'text': self.prompt}]
        t0 = time.time()
        try:
            response = await self.client.chat.completions.create(model=self.model, messages=[{'role': 'user', 'content': content}], max_tokens=8096, temperature=0.0)
            text = response.choices[0].message.content or ''
            text = text.strip()
            elapsed = time.time() - t0
            tokens = getattr(response.usage, 'completion_tokens', 0) if response.usage else 0
            tps = tokens / elapsed if elapsed > 0 else 0
            ocr_parser_extractor_log.info(f'  стр.{page_num}: {tokens} tok, {elapsed:.1f}s, {tps:.0f} tok/s')
            self._log_page(page_num, text)
            return text
        except Exception as e:
            elapsed = time.time() - t0
            ocr_parser_extractor_log.error(f'  стр.{page_num}: ошибка за {elapsed:.1f}s — {e}')
            raise
    ...
    def _pdf_to_images(self, pdf_path: str) -> List[Image.Image]:
        ...
        ocr_parser_extractor_log.info(f'PDF -> {len(images)} страниц ({self.dpi} dpi)')
        return images
...
ocr_parser_extractor_module = SimpleNamespace(log=ocr_parser_extractor_log, OcrExtractor=OcrExtractor)

# END_SOURCE_OCR_PARSER_EXTRACTOR
...
# START_SOURCE_OCR_PARSER
# PURPOSE: Inlined source from audit_engine/ocr_parser/__init__.py.
ocr_parser_log = logging.getLogger(__name__)

ocr_parser___all__ = ['OcrParser']
...
class OcrParser:
    ...
    def __init__(self, config: AuditConfig, log_dir: Optional[str]=None):
        ...
        self.extractor = OcrExtractor(base_url=config.ocr_base_url, model=config.ocr_model, prompt=config.ocr_prompt, dpi=config.ocr_dpi, log_dir=log_dir)
    ...
    def parse(self, file_path: str, chunk_filter: Optional[str]=None) -> Dict[str, Any]:
        ...
        try:
            ...
            if effective_total != total_pages:
                ocr_parser_log.info(f'Страниц в PDF: {total_pages}, эффективных: {effective_total} (пустые: {sorted((i + 1 for i in blank_pages))})')
            ...
            ocr_parser_log.info(f'Уникальных страниц для OCR: {len(unique_page_indices)} из {total_pages}')
            page_texts = self.extractor.extract_pages(pdf_path, unique_page_indices)
            chunk_texts = self.assembler.assemble(page_texts, chunks, effective_total, strip_annotations_chunks=self.config.strip_annotations_chunks)
...
ocr_parser_module = SimpleNamespace(log=ocr_parser_log, __all__=ocr_parser___all__, OcrParser=OcrParser)

# END_SOURCE_OCR_PARSER
...
class AuditEngine:
    ...
    def _parse_document(self, file_path: str, chunks_to_parse: Optional[List[str]], vision_log_dir: Path) -> Dict[str, Any]:
        ...
        if parser_name == 'docx':
            ...
            vision_parser = VisionParser(str(self.config.chunks_vision_path), log_dir=str(vision_log_dir))
