...
from src.format_parsers.pdf import load_pdf_parser_config, parse_pdf
...
class AuditEngine:
    ...
    def _parse_document(self, file_path: str, chunks_to_parse: Optional[List[str]], vision_log_dir: Path) -> Dict[str, Any]:
        ...
        if parser_name == 'docx':
            ...
            doc = parse_docx(file_path, vlm_base_url=self.pdf_parser_config.vlm.base_url)
...
def _audit_engine_init(self, doc_type: str, base_dir: Optional[str] = None, config_dir: Optional[str] = None) -> None:
    ...
    # Инфра-конфиг PDF-парсера — общий для всех doc_types, читаем один раз при инициализации.
    self.pdf_parser_config = load_pdf_parser_config()
    self.system_prompt = self._load_system_prompt()
    self.preprocessors = get_preprocessors(doc_type)
    self.logger = None
