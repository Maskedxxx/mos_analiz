#!/usr/bin/env python3
"""
Валидатор 1.6: Дата реализации должна быть периодом
Источник ТЗ: ТЗ_Валидация_КПСЦ_v2_упрощенная.md, Раздел 1, Проверка 1.6
"""
import os
import json
import argparse
...
# Константы
RULE_INDEX = "1.6"
RULE_TITLE = "Дата реализации должна быть периодом"
...
def extract_relevant_data(data: dict) -> dict:
    ...
    return {
        "date_implementation": data["fields"].get("date_implementation", "")
    }
...
def main():
    ...
    if args.verbose:
        log_step(log_file, 2, "Загрузка данных", f"Source file: {args.parser_outputs / 'kpsc_header_v2.json'}\nData loaded:\n{json.dumps(data, ensure_ascii=False, indent=2)}")
